...
logfile=os.path.dirname(os.path.realpath(__file__))+'\\logs\\log'+dt_string+'.txt'
logf = open(logfile, "w+")
logf.write('Magneto Bot')
logf.close()
logf = open(logfile, "a")

def debug(message,logf=logf):
    if debugging == 1:
        print(message)
    if logging == 1:
...
def right_click_delay(x,y,t1,t2):
    time.sleep(random.uniform(t1,t2))
    win32api.SetCursorPos((x,y))
    win32api.mouse_event(win32con.MOUSEEVENTF_RIGHTDOWN,0,0)
    win32api.mouse_event(win32con.MOUSEEVENTF_RIGHTUP,0,0)
...
def action_attack(character):
    ...
    if location:
        ...
        #_thread.start_new_thread(ability_unit_target,(location[0]+10, location[1]+50,'a'))
...
def action_run_away():
    location = None
    red_hero_locations = game_screen.parsed_red_hero_locations()
    debug('action_run_away, red hero locations')
    debug(red_hero_locations)

    if red_hero_locations:
        location = None
        debug('Found heros running away')
        ...
        if location != None:
            _thread.start_new_thread(right_click,(location[0]+10,location[1]+50))
            _thread.start_new_thread(right_click_delay,(location[0]+10,location[1]+50,.5,1))
            _thread.start_new_thread(right_click_delay,(location[0]+10,location[1]+50,1.1,1.6))
            _thread.start_new_thread(right_click_delay,(location[0]+10,location[1]+50,2.2,2.7))
            _thread.start_new_thread(right_click_delay,(location[0]+10,location[1]+50,3.3,3.8))
...
def action_stick_team_screen():
    ...
    debug('trying to stick with team on screen')
    debug(ally_locations)
    if ally_locations:
        _thread.start_new_thread(right_click,(ally_locations[0][0], ally_locations[0][1]))
    else:
        debug('no allies, just moving in direction of red core.')
        action_walk_red_core()

def action_walk_red_core():
        location = None
        if game_screen.blue_side == 'left':     
            location = ((random.randrange(1700,1800)), random.randrange(400,450))
        if game_screen.blue_side == 'right':
...
def action_find_team():
    ...
    if ally_location != None:
        debug('found ally on minimap')
        win32api.SetCursorPos((ally_location[0]+15,ally_location[1]+15))
        pyautogui.hotkey('a')
    else:
        debug('no ally on minimap')
def action_hearth():
    red_hero_locations = game_screen.parsed_red_hero_locations()
    if not red_hero_locations:
        _thread.start_new_thread(hit_key,('b',dt1,dt2))
    #hero_object_1.ClockChannel = clock_timer(6)
...
class check_game():
    def __init__(self):
        self.blue_side = None
        self.blue_keep_fort = None
        self.red_keep_fort = None
        ...
        if self.blue_side == 'left':
            self.blue_keep_fort = self.parse_locations(pyautogui.locateAllOnScreen('blue_keep_left.png', confidence=0.7))
        else:
            self.blue_keep_fort = self.parse_locations(pyautogui.locateAllOnScreen('blue_keep_right.png', confidence=0.7))
        if self.blue_side == 'right':
            self.red_keep_fort = self.parse_locations(pyautogui.locateAllOnScreen('red_keep_left.png', confidence=0.8))
        else:
            self.red_keep_fort = self.parse_locations(pyautogui.locateAllOnScreen('red_keep_right.png', confidence=0.8))
        
        
        debug('Blue Keep and for locations')
        for a in self.blue_keep_fort:
            debug(a)
        debug('Red Keep and for locations')
        for a in self.red_keep_fort:
            debug(a)
    ...
    def parsed_blue_hero_locations(self):
        return self.parse_locations(pyautogui.locateAllOnScreen('ally_health_bar.png', confidence=0.9))
    def parsed_red_hero_locations(self): 
        return self.parse_locations(pyautogui.locateAllOnScreen('enemy_health_bar.png', confidence=0.9))
    ...
    def read_ally_healthbars(self):
        ...
        debug('hero locations:')
        debug(hero_locations)
        if not hero_locations:
            return None
        else:
    ...
    def find_lowest_ally_healthbar(self):
        ...
        debug('ally_healthbars')
        debug(ally_healthbars)
        if not ally_healthbars:
            debug('No ally healthbars')
            return None
        else:
        ...
        if not locations:
            ...
            for location in locations:
                if locations != None:
                    ...
                    #    parsed_locations.append((location.left, location.top))
                    #else:
                    #    lastlocation = parsed_locations.pop()
                    #    distance = abs(lastlocation[0] - location.left)+abs(lastlocation[1] - location.top)
                    #    if distance > 10:
                    #        parsed_locations.append((location.left, location.top))
                    #    else:
                    #        count = count + 1
                    #        debug('parsed locations removed')
                    #        debug('distance is')
                    #        debug(distance)
                            
                    #        debug( count )
            debug('returning parsed locations:')   
            debug(parsed_locations)
...
class hero_object:
    def __init__(self,
        ...
        self.wait = 'off'
        self.mount = 'off'
        self.ClockChannel = clock_timer(1)
...
def run_action(character):
        ...
        if character.stick_team == 'on':
            dlowest_ally = game_screen.find_lowest_ally_healthbar()
            if not dlowest_ally:
                debug('No allies on screen running action_find_team')
                #if character.ClockFind_Ally.check_timer():
                action_find_team()
            else:
                debug('Found someone on screen stayin still')
                debug(dlowest_ally)
...
while keyboard.is_pressed('p') == False:
    ...
    if gamemode == 1:
        ...
        if hero_1 == 'default':
            ...
            print('time between ai rotation')
            
            
            run_ai(hero_object_1)
            print('time between ai rotation')
            print(time.time() - ai_time)
            ai_time = time.time()
